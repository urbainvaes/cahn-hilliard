...
parser.add_argument('label', type=str, help='label used when saving pictures')
parser.add_argument('-f', '--flow', action='store_true', help='include pressure and velocity')
parser.add_argument('-m', '--mesh', action='store_true', help='draw triangulation')
parser.add_argument('-p', '--parallel', action='store_true', help='run in parallel')
parser.add_argument('-e', '--extension', type=str, default='pdf', help='extension of output files')
parser.add_argument('-i', '--iteration', type=str, help='iteration number')
parser.add_argument('-s', '--step', type=int, default=1, help='step for iterations')
parser.add_argument('--input', type=str, default="output", help='directory of data files')
parser.add_argument('--output', type=str, default="pictures", help='directory for figure files')
...
fields = ['phi', 'mu']
if args.flow: fields += ['pressure', 'velocity']
for f in fields:
...
def plot_iteration(iteration):
  ...
  data = {}
  for f in fields:
    data[f] = mesh.read_data(args.input + '/' + f + '/' + f + '-' + str(iteration) + '.msh')
  ...
  for f in fields:
    if f == 'phi':
      tricontourf = plt.tricontourf(triangulation, data[f], 40, cmap = 'blue_green', extend='both')
    elif f == 'mu':
      tricontourf = plt.tricontourf(triangulation, data[f], 40, cmap = 'jet')
    elif f == 'pressure':
      tricontourf = plt.tricontourf(triangulation, data[f], 40, cmap = 'jet')
    elif f == 'velocity':
      vx = [data[f][i][0] for i in range(len(data[f]))]
      vy = [data[f][i][1] for i in range(len(data[f]))]
      abs_v  = [np.sqrt(vx[i]*vx[i] + vy[i]*vy[i]) for i in range(len(data[f]))]
      tricontourf = plt.tricontourf(triangulation, abs_v, 40)
    colorbar = plt.colorbar(tricontourf, orientation = c_orientation, fraction = 0.05, pad = 0.05)
    ...
    output = args.output + '/' + f + '/' + args.label + '-' + f + '-' + str(iteration) + '.' + args.extension
    plt.xticks([])
    plt.yticks([])
    plt.show()
